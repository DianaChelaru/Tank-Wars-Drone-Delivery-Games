...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    float zoomSpeed = deltaTime * 0.5f;

    if (window->KeyHold(GLFW_KEY_W)) {
        logicSpace.y += moveSpeed * deltaTime;
    }
    if (window->KeyHold(GLFW_KEY_S)) {
        logicSpace.y -= moveSpeed * deltaTime;
    }
    if (window->KeyHold(GLFW_KEY_A)) {
        logicSpace.x -= moveSpeed * deltaTime;
    }
    ...
    if (window->KeyHold(GLFW_KEY_Z))  // Zoom in
    {
        logicSpace.width -= deltaTime / 2.0f;
        logicSpace.height -= deltaTime / 2.0f;
        logicSpace.x += deltaTime / 4.0f;
        logicSpace.y += deltaTime / 4.0f;
    }

    if (window->KeyHold(GLFW_KEY_X))  // Zoom out
    {
        logicSpace.width += deltaTime / 2.0f;
        logicSpace.height += deltaTime / 2.0f;
        logicSpace.x -= deltaTime / 4.0f;
