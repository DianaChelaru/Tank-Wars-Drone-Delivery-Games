...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(1, 0.2, 0.7));
...

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.6, 0.2, 1));
...

    {
        ...
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.7, 0.4, 0));
    }
...
void Lab8::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, const glm::vec3& color)
{
    ...
    glUniform1f(cut_off_loc, glm::cos(cutOff));
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_1)) {
            cutOff += deltaTime * RADIANS(50);
            cutOff = glm::min(cutOff, RADIANS(180.0f));
        }
        if (window->KeyHold(GLFW_KEY_2)) {
            cutOff -= deltaTime * RADIANS(50);
            cutOff = glm::max(cutOff, 0.0f);
        }
